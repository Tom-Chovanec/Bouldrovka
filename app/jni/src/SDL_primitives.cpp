...
bool isPointInRoundedRect(int x, int y, SDL_Rect rect, int radius) {
    // Check if the point is inside the rectangle excluding the top corners
    if ((x >= rect.x + radius && x < rect.x + rect.w - radius) ||
        (y >= rect.y + radius && y < rect.y + rect.h)) {
        return true;
    }

    // Check if the point is inside the top corners
    int dx, dy;
    // Top-left corner
    dx = rect.x + radius - x;
    dy = rect.y + radius - y;
    ...
    // Bottom corners are excluded from the check
